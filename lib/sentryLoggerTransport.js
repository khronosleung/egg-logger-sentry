...
const fillScopeBaseProps = require('./utils/fillScopeBaseProps');
...
function eggLogLevelToSentryLevel(level, Sentry) {
  switch (level) {
    case 'debug':
...
class SentryLoggerTransport extends Transport {
  log(level, args, meta) {
    ...
    if (is.undefined(ctx.sentryScope)) {
    ...
    if (is.object(options.extras)) {
      keys(options.extras).forEach(key => {
    ...
    scope.setLevel(eggLogLevelToSentryLevel(level, Sentry));
    if (ctx && ctx.request) {
    ...
    if (level === Sentry.Severity.Error.toLowerCase()) {
